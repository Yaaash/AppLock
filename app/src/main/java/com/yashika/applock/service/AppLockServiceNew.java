...
import com.yashika.applock.AppLockActivity;
...
public class AppLockServiceNew extends Service {

    /**
     * Sent to {@link } when the service has been completely started and is running
     */
    public static final String BROADCAST_SERVICE_STARTED = "com.twinone.locker.intent.action.service_started";
    /**
     * Sent to {@link } when the service has been stopped
     */
    public static final String BROADCAST_SERVICE_STOPPED = "com.twinone.locker.intent.action.service_stopped";
    /**
     * This category allows the receiver to receive actions relating to the state of the service,
     * such as when it is started or stopped
     */
    ...
    /**
     * When specifying this action, the service will initialize everything again.<br> This has only
     * effect if the service was explicitly started using {@link #getRunIntent(Context)}
     */
    ...
    List<ActivityManager.RunningTaskInfo> mTestList = new ArrayList<ActivityManager.RunningTaskInfo>();
    ...
    /**
     * This map contains locked apps in the form<br> <PackageName, ShortExitEndTime>
     */
     ...
     *
     * @return True if all OK, false if the service is not allowed to start (the caller should stop
     * the service)
     */
    private boolean init() {
        ...
        final List<String> apps = SessionPreferences.INSTANCE.getLockedApps();
        for(String s : apps) {
...
//                                      R.string.pref_def_delay_time);
        mShortExitMillis = 600000;
//        }

        mRelockScreenOff = true;
//                = prefs.getBoolean(
...
//            s = "0";
    ...
    private void checkPackageChanged() {
        final ActivityManager.RunningTaskInfo top = getTopTask();
